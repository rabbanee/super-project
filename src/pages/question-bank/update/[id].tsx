...
const UpdateQuestions = () => {
  ...
  const getQuestion = async () => {
    let response;
    try {
      response = await axios.get(`${process.env.NEXT_PUBLIC_API_HOST}question/${id}`, {
        headers: {
    ...
    if (question.type === 'Multiple Choice') {
      const trueOption: any = question.options.find((option) => option.is_true);
      setSelectedTypeOfQuestion('Pilihan Ganda');
      console.log('trueOption: ', trueOption);
    } else {
      setSelectedTypeOfQuestion('Esai');
    }
  ...
  const addQuestion = async (e: any) => {
    ...
    try {
      response = await axios.put(`${process.env.NEXT_PUBLIC_API_HOST}question/${id}`, payload, {
        headers: {
          'Authorization': `Bearer ${token}`,
        }
      });
  ...
  return (
    <LayoutWithSidebar user={user} title="Edit Soal" permissions={permissions.list}>
      <Container>
        <form onSubmit={addQuestion} ref={formRef}>
          <ContainerBody>
            <div className="flex justify-between flex-wrap">
              <h2 className="text-3xl font-bold	text-black mb-2">Edit Soal</h2>
            ...
            <div className="grid grid-cols-2 gap-3 mb-5">
              <div className="col-span-2">
                ...
                <Editor onEditorChanges={onEditorChanges} data={question.content || content || ''}/>
              </div>
              <div className="col-span-2">
              ...
              </div>
              {
                ...
                (
                  <>
                    <div className="col-span-2 sm:col-span-1">
                      ...
                      <textarea name="option_a" id="option_a" cols={20} rows={8} className="appearance-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-primary-dark focus:border-primary-dark focus:z-10 sm:text-sm" defaultValue={question?.options[0]?.content}>
                      </textarea>
                    </div>
                    <div className="col-span-2 sm:col-span-1">
                      <label htmlFor="option_b" className="block text-md font-medium text-gray-700">Opsi B</label>
                      <textarea name="option_b" id="option_b" cols={20} rows={8} className="appearance-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-primary-dark focus:border-primary-dark focus:z-10 sm:text-sm" defaultValue={question?.options[1]?.content}>
                      </textarea>
                    </div>
                    <div className="col-span-2 sm:col-span-1">
                      <label htmlFor="option_c" className="block text-md font-medium text-gray-700">Opsi C</label>
                      <textarea name="option_c" id="option_c" cols={20} rows={8} className="appearance-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-primary-dark focus:border-primary-dark focus:z-10 sm:text-sm" defaultValue={question?.options[2]?.content}>
                      </textarea>
                    </div>
                    <div className="col-span-2 sm:col-span-1">
                      <label htmlFor="option_d" className="block text-md font-medium text-gray-700">Opsi D</label>
                      <textarea name="option_d" id="option_d" cols={20} rows={8} className="appearance-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-primary-dark focus:border-primary-dark focus:z-10 sm:text-sm" defaultValue={question?.options[3]?.content}>
                      </textarea>
                    </div>
                  </>
                ) :
                ...
                <div className="col-span-2 sm:col-span-2">
                  ...
                  <textarea name="essay_correct_answer" id="essay_correct_answer" cols={20} rows={8} className="appearance-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-primary-dark focus:border-primary-dark focus:z-10 sm:text-sm" defaultValue={question?.essay_answer?.content || ''}>
                  </textarea>
                </div>
              }
...
export default WithAuth(UpdateQuestions, 'crud question bank');
